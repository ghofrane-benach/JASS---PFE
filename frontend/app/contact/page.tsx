...
const FAQ = [
  { q: "Combien de temps pour obtenir une réponse ?",   a: "Nous répondons dans les 24h ouvrables. Pour les urgences, mentionnez 'URGENT' dans votre message." },
  { q: "Puis-je retourner ou échanger un produit ?",    a: "Oui, vous avez 5 jours à compter de la réception pour retourner un produit en parfait état." },
  { q: "Comment modifier ma commande ?",                a: "Vous pouvez modifier votre commande dans les 2 heures suivant la confirmation. Contactez-nous avec votre numéro de commande." },
];
    ...
    <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", background: '#fff', color: '#111', minHeight: '100vh' }}>
      ...
      <section style={{ background: '#080808', color: '#fff', padding: '110px 6vw 90px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 700, margin: '0 auto', position: 'relative' }}>
          <p style={{ fontSize: 10, letterSpacing: '0.5em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)', marginBottom: 24 }}>JASS — Contactez-nous</p>
      ...
      <section style={{ padding: '90px 6vw' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 80, alignItems: 'start' }}>
          ...
          <div>
            ...
            {submitted ? (
              <div style={{ padding: '48px 32px', border: '1px solid #4a9e6f', textAlign: 'center', animation: 'fadeUp 0.4s ease' }}>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <Field label="Votre nom"  id="name"  name="name"  value={form.name}  onChange={handleChange} placeholder="Your name." />
                  <Field label="Email *"    id="email" name="email" value={form.email} onChange={handleChange} placeholder="yourName@jass.com" type="email" required />
                </div>
                ...
                <div>
                  ...
                  <textarea id="message" name="message" value={form.message} onChange={handleChange} rows={6} required
                    ...
                    style={{ width: '100%', padding: '16px', resize: 'vertical', border: '1px solid #e8e8e8', outline: 'none', fontFamily: 'inherit', fontSize: 14, lineHeight: 1.7, color: '#333', background: '#fafafa', boxSizing: 'border-box', transition: 'border-color 0.2s' }}
                    onFocus={e => (e.target.style.borderColor = '#111')}
                    onBlur={e  => (e.target.style.borderColor = '#e8e8e8')}
                  />
                </div>

                {error && <p style={{ fontSize: 12, color: '#c0392b', letterSpacing: '0.04em', margin: 0 }}>⚠ {error}</p>}
                ...
                <button type="submit" disabled={loading} style={{
                  ...
                  {loading ? (
                    <><div style={{ width: 14, height: 14, border: '1px solid rgba(255,255,255,0.4)', borderTop: '1px solid #fff', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} />Envoi en cours…</>
                  ) : 'Envoyer le message'}
                </button>
              </form>
            )}
          ...
          {/* INFO CARDS (inchangées) */}
          <div style={{ paddingTop: 64 }}>
            <p style={{ fontSize: 10, letterSpacing: '0.4em', textTransform: 'uppercase', color: '#aaa', marginBottom: 14 }}>Coordonnées</p>
            <h2 style={{ fontSize: 'clamp(1.4rem, 2.5vw, 2rem)', fontWeight: 300, margin: '0 0 40px' }}>Retrouvez-nous</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              ...
              ].map((info, i) => <InfoCard key={i} {...info} />)}
            </div>
            <div style={{ marginTop: 32, paddingTop: 32, borderTop: '1px solid #f0f0f0' }}>
              <p style={{ fontSize: 10, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#aaa', marginBottom: 16 }}>Suivez-nous</p>
      ...
      <section style={{ padding: '90px 6vw', background: '#f9f9f9' }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            {FAQ.map((faq, i) => (
              <FaqItem key={i} question={faq.q} answer={faq.a} open={openFaq === i} onToggle={() => setOpenFaq(openFaq === i ? null : i)} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 6vw', textAlign: 'center' }}>
        ...
        <Link href="/products" style={{ display: 'inline-block', padding: '15px 52px', background: '#111', color: '#fff', textDecoration: 'none', fontSize: 11, letterSpacing: '0.25em', textTransform: 'uppercase', fontFamily: 'inherit' }}>
          Voir la collection
        </Link>
      </section>
    </div>
...
function Field({ label, id, name, value, onChange, placeholder, type = 'text', required = false }: { label: string; id: string; name: string; value: string; onChange: (e: any) => void; placeholder?: string; type?: string; required?: boolean }) {
  return (
    <div>
      <label htmlFor={id} style={{ display: 'block', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#888', marginBottom: 10 }}>{label}</label>
      <input id={id} name={name} type={type} value={value} onChange={onChange} placeholder={placeholder} required={required}
        style={{ width: '100%', padding: '14px 16px', border: '1px solid #e8e8e8', background: '#fafafa', outline: 'none', fontFamily: 'inherit', fontSize: 14, color: '#333', boxSizing: 'border-box', transition: 'border-color 0.2s' }}
        onFocus={e => (e.target.style.borderColor = '#111')}
        onBlur={e  => (e.target.style.borderColor = '#e8e8e8')}
      />
    </div>
...
function InfoCard({ icon, title, lines }: { icon: string; title: string; lines: string[] }) {
  ...
  return (
    <div style={{ padding: '20px 24px', background: '#fff', border: '1px solid', borderColor: hov ? '#111' : '#f0f0f0', display: 'flex', alignItems: 'flex-start', gap: 16, transition: 'border-color 0.25s' }}
...
function SocialBtn({ href, label }: { href: string; label: string }) {
  ...
  return (
    <a href={href} target="_blank" rel="noopener noreferrer"
      style={{ padding: '10px 24px', border: '1px solid', textDecoration: 'none', borderColor: hov ? '#111' : '#e8e8e8', color: hov ? '#111' : '#888', fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', fontFamily: 'inherit', transition: 'all 0.2s' }}
      onMouseOver={() => setHov(true)} onMouseOut={() => setHov(false)}>
      {label}
    </a>
  );
...
function FaqItem({ question, answer, open, onToggle }: { question: string; answer: string; open: boolean; onToggle: () => void }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #f0f0f0', overflow: 'hidden' }}>
      <button onClick={onToggle} style={{ width: '100%', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left' }}>
        <span style={{ fontSize: 15, fontWeight: 400, color: '#111', lineHeight: 1.4 }}>{question}</span>
        <span style={{ fontSize: 18, color: '#aaa', marginLeft: 16, flexShrink: 0, transform: open ? 'rotate(45deg)' : 'none', transition: 'transform 0.25s', display: 'inline-block' }}>+</span>
      </button>
      {open && (
